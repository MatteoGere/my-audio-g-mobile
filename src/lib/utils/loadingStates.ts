...
export const useBatchLoading = <T extends { id: string | number }>() => {
  ...
  const setItemLoading = useCallback((id: string | number, isLoading: boolean, message?: string) => {
    setItemStates(prev => ({
      ...prev,
      [id]: {
        isLoading,
  ...
  const startItemLoading = useCallback((id: string | number, message?: string) => {
    setItemLoading(id, true, message);
  }, [setItemLoading]);

  const stopItemLoading = useCallback((id: string | number) => {
  ...
  const getItemState = useCallback((id: string | number) => {
